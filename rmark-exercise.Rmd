...
<!-- rmark-exercise.Rmd - Rmarkdown code for the Rmarkdown exercise. The "<--"
     is how you start and end a comment (it is the .html format for comments).
-->

# Simple Rmarkdown exercise
<!-- The # here is Rmarkdown for 'Section Heading' -->

Run this code by clicking the `knitr` button in RStudio (TODO YES???) or
...
file. Comments for understanding will be done in the `.Rmd` file and should be
read. We will make the resulting output in the `.html` file focus on the data
...
#  is the default. You can customise pretty much anything (see the Rmarkdown
#  book). It also caches the output -- more about that later.
knitr::opts_chunk$set(comment = "",
                      cache = TRUE)
```

<!-- Note that because the above chunk is R code, the `#` symbol represents a
comment. Your text editor should hopefully colour code this file so you can
distinguish Rmarkdown headings and R comments. -->
...
<!-- The ` r` at
     the start of each of these tells Rmarkdown to evaluate the remaining code in
...
tables) with the `kable` and `kableExtra` packages (see the Rmarkdown book), so
