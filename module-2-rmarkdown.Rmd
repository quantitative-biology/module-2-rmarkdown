...
1. Download [rmark-exercise.Rmd](https://raw.githubusercontent.com/quantitative-biology/module-2-rmarkdown/main/rmark-exercise.Rmd)
...
1. Check that this has produced ('rendered') an `.html` file document that looks
   ...
   available on the [GitHub repository for this module](https://github.com/quantitative-biology/module-2-rmarkdown)).
...
Mention no need for multiple output formats **
...
Now download
[rmark-pdf-example.Rmd](https://raw.githubusercontent.com/quantitative-biology/module-2-rmarkdown/main/rmark-pdf-example.Rmd)
onto your computer and put it where you want to run it. As before, make
sure it is saved as `rmark-pdf-example.Rmd` and not `rmark-pdf-example.Rmd`.

Try running it as earlier, either using `rmarkdown::render("rmark-pdf-example.Rmd")`
in R, or clicking the `knitr` button in RStudio but selecing the `knit-as-pdf` option. If it doesn't run then you
...
Using those warnings as a teachable moment, this is why I set `keep_tex = TRUE` in the
