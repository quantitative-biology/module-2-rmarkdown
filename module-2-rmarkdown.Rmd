...
online [Definitive Guide to R Markdown](https://bookdown.org/yihui/rmarkdown ).
...
```{r, echo = FALSE, fig.alt = "Two example pages showing multiple figures of data.", width="100%"}
knitr::include_graphics("silvergray.jpg")
```
...
1. The file is an R Markdown (`.Rmd`) file that you can run by either clicking
...
1. Check that this has produced ('rendered') an `.html` file document that looks
   ...
   file, `module-2-rmarkdown.Rmd`, used to create what you are reading is available on the [GitHub
   repository for this
   module](https://github.com/quantitative-biology/module-2-rmarkdown)).
...
the simple instructions another book by Yhiui Xie et al., the [R Markdown
...
R Markdown](https://bookdown.org/yihui/rmarkdown) is highly recommended (and
...
2](https://bookdown.org/yihui/rmarkdown/basics.html) goes through the basics,
including many things that we glossed over above, such as the code between the
`---` at the top of `rmark-exercise.Rmd`. For me to explain it properly, I would
have to refer to the guidebook anyway.
...
these materials in [this survey](https://forms.gle/vLkcZq7NxPb2ajTQ6).
