...
<!-- This is a comment. The above packages are things that we have found useful,
     in particular mathptmx gives a bolder font than the Latex default.
...
where
\eb
\epsilon_i \sim \mbox{Normal}(0, 10).
\ee

The maximum value of $y$ is `r max(y)`, or rounded to two decimal places is
`r round(max(y),2)`.
